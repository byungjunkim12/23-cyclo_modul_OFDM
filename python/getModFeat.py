...
pi = math.pi
def getModFeatNR(inputIQ, nSubC, lenCP, firstIndexSym, nSym, removeNull, angleMod):
...
def getModFeatWifi(inputIQ, nSubC, lenCP, firstIndexSym, nSym, removeNull, angleMod):
    featAbs = np.zeros((nSubC, nSym))
    featPh = np.zeros((nSubC, nSym))
    if nSubC == 64:
        nNullSubC = 8
